...
function removeExpiredLicenses($db) {
    ...
    if (!empty($expiredLicenses)) {
        ...
        $updateExpired = $db->prepare("UPDATE license SET hwid = 'expiryed' WHERE license IN ($inClause)"); 
        $updateExpired->execute($expiredLicenseIDs);
...
if (isset($_POST["generate_random_license"])) {
    $license_count = intval($_POST["license_count"]); // Kaç lisans oluşturulacağını al
    $license_days = intval($_POST["manuald_days"]); // Lisans süresini al

    $generatedLicenses = generateRandomLicenses($db, $license_count, $license_days);

    // Lisansları notepad dosyasına yaz
    $notepadContent = implode("\n", $generatedLicenses);
    $notepadFilename = "generated_licenses.txt";

    // Dosyayı oluştur ve içeriği yaz
    $file = fopen($notepadFilename, "w");
    fwrite($file, $notepadContent);
    fclose($file);

    // Dosyayı kullanıcıya indirme başlat
    header("Content-disposition: attachment; filename=$notepadFilename");
    header("Content-type: text/plain");
    readfile($notepadFilename);
    exit;
}

function generateRandomLicenses($db, $count, $license_days) {
    $generatedLicenses = [];

    for ($i = 0; $i < $count; $i++) {
        $randomLicense = generateRandomLicense();

        // Oluşturulan lisansın veritabanında olup olmadığını kontrol et
        $query = $db->prepare("SELECT * FROM license WHERE license = :license");
        $query->execute(['license' => $randomLicense]);
        $result = $query->fetch(PDO::FETCH_ASSOC);

        // Eğer lisans veritabanında yoksa, lisansı ekleyin
        if (!$result) {
            $insert = $db->prepare("INSERT INTO license (license,expiryend) VALUES (:license,:expiryend)");
            $insert->execute([
            ...
            // Oluşturulan lisansı listeye ekleyin
            $generatedLicenses[] = $randomLicense;
        }
    }
...
<head>
    ...
    <link rel="icon" type="image/png" href="https://cdn.icon-icons.com/icons2/1860/PNG/512/key_118055.png">
...
<body>
    ...
    <div class="main-container">
        ...
        <form action="" method="post">
            ...
            <label class="form-label" for="license_count">Kaç lisans oluşturmak istiyorsunuz?</label>
            <input class="form-input" type="number" id="license_count" name="license_count" placeholder="1" required>
            
            <label class="form-label" for="license_duration">Lisansların süresini girin (Gün):</label>
            <input class="form-input" type="number" id="license_duration" name="manuald_days" placeholder="1" required>
            
            <button class="form-submit" type="submit" name="generate_random_license">Rastgele Lisansları Oluştur</button>
        </form>
    </div>
</div>
        </div>
        <div class="license-container">
            <div class="table-container">
                <h2 class="license-header">LISANSLAR</h2>
                <div class="table-responsive">
                    <table class="table table-bordered">
                       ...
                       <tbody>
                            ...
                            ?>
                            <tr>
 <?php if ($licenseInfo['hwid'] !== 'Banned' && $licenseInfo['hwid'] !=='expiryed') { ?>
                                <td><?php echo $licenseInfo['license']; ?></td>
 <?php if ($licenseInfo['hwid'] !== 'expiryed') {?>
                                <td><?php echo "Yasaklanmadı"; ?></td>
 <?php } else {?>
                                <td><?php echo "SURESI DOLDU"; ?></td>
 <?php }?>
   <?php if ($licenseInfo['hwid'] !== 'expiryed') {?>
 ...
 <?php } else {?>
<td><?php echo " "; ?></td>
     <td><?php  echo "0 Ay,0 Gün,0 Saat,0 Dakika"; ?></td>
 <?php }?>                                
<td>
                                    <form class="d-inline" method="post" action="">
                                        <input type="hidden" name="delete_license" value="<?php echo $licenseInfo['license']; ?>">
                                        <button class="btn btn-danger" type="submit">Lisansı Sil</button>
                                    </form>
   <?php if ($licenseInfo['hwid'] !== 'expiryed') {?>
                                    <form class="d-inline" method="post" action="">
                                    ...
                                    <form class="d-inline" method="post" action="">
                                        ...
                                        <button class="btn btn-danger" type="submit">Yasakla</button>
                                    </form>
 <?php } ?>
                                </td>
 <?php }   ?>
    ...
    <div class="table-container2">
        <h2 class="license-header">YASAKLI KULLANICILAR</h2>
        <div class="table-responsive">
            <table class="table table-bordered">
                        <thead>
                            <tr>
 ...
 <?php if ($licenseInfo['hwid'] === 'Banned') { ?>
                                ...
                                <td>
                                    ...
                                    <form class="d-inline" method="post" action="">
                                        <input type="hidden" name="ban_license" value="<?php echo $licenseInfo['license']; ?>">
                                       <button class="btn btn-success" type="submit">Yasağı Kaldır</button>
                                    </form>
                                </td>
 <?php }   ?>
                            </tr>
...
<div class="table-container2">
        <h2 class="license-header">SURESI DOLMUS LISANSLAR</h2>
        <div class="table-responsive">
            <table class="table table-bordered">
                        <thead>
                            <tr>
